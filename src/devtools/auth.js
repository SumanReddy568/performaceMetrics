...
async function storeAuthData(token, email, hash) {
  ...
  // Also store in chrome storage if available (for extension contexts)
  if (typeof chrome !== "undefined" && chrome.storage) {
    try {
      await chrome.storage.local.set({
        ...
        user_email: email,
        user_hash: hash,
        user_id: email, // Use email as user_id or generate a proper user_id from your API
      });
    } catch (e) {
      console.warn("Failed to store auth data in chrome storage:", e);
    }
  }

  // Import and call setUserInfo if analytics module is available
  try {
    const { setUserInfo } = await import("../../utils/analytics.js");
    await setUserInfo(email, email, hash); // Using email as user_id, adjust as needed
  } catch (e) {
    console.warn("Failed to sync user info with analytics:", e);
  }
}
...
async function clearAuthData() {
  ...
  if (typeof chrome !== "undefined" && chrome.storage) {
    try {
      ...
      console.warn("Failed to clear auth data from chrome storage:", e);
    }
  }

  // Clear analytics user info
  try {
    const { setUserInfo } = await import("../../utils/analytics.js");
    await setUserInfo(null, null, null);
  } catch (e) {
    console.warn("Failed to clear user info from analytics:", e);
  }
}
...
async function signup(email, password) {
  try {
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      throw new Error(
        `Server returned unexpected format (Status: ${response.status}). Please check API URL.`
      );
    }
